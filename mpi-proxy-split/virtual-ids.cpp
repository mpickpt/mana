...
void init_lh_constants_map() {
  lh_constants_map[MPI_GROUP_NULL] = REAL_CONSTANT(GROUP_NULL);
  lh_constants_map[MPI_COMM_NULL] = REAL_CONSTANT(COMM_NULL);
  lh_constants_map[MPI_REQUEST_NULL] = REAL_CONSTANT(REQUEST_NULL);
  lh_constants_map[MPI_MESSAGE_NULL] = REAL_CONSTANT(MESSAGE_NULL);
  lh_constants_map[MPI_OP_NULL] = REAL_CONSTANT(OP_NULL);
  lh_constants_map[MPI_ERRHANDLER_NULL] = REAL_CONSTANT(ERRHANDLER_NULL);
  lh_constants_map[MPI_INFO_NULL] = REAL_CONSTANT(INFO_NULL);
  lh_constants_map[MPI_WIN_NULL] = REAL_CONSTANT(WIN_NULL);
  lh_constants_map[MPI_FILE_NULL] = REAL_CONSTANT(FILE_NULL);
  lh_constants_map[MPI_INFO_ENV] = REAL_CONSTANT(INFO_ENV);
  lh_constants_map[MPI_COMM_WORLD] = REAL_CONSTANT(COMM_WORLD);
  lh_constants_map[MPI_COMM_SELF] = REAL_CONSTANT(COMM_SELF);
  lh_constants_map[MPI_GROUP_EMPTY] = REAL_CONSTANT(GROUP_EMPTY);
  lh_constants_map[MPI_MESSAGE_NO_PROC] = REAL_CONSTANT(MESSAGE_NO_PROC);
  lh_constants_map[MPI_MAX] = REAL_CONSTANT(MAX);
  lh_constants_map[MPI_MIN] = REAL_CONSTANT(MIN);
  lh_constants_map[MPI_SUM] = REAL_CONSTANT(SUM);
  lh_constants_map[MPI_PROD] = REAL_CONSTANT(PROD);
  lh_constants_map[MPI_LAND] = REAL_CONSTANT(LAND);
  lh_constants_map[MPI_BAND] = REAL_CONSTANT(BAND);
  lh_constants_map[MPI_LOR] = REAL_CONSTANT(LOR);
  lh_constants_map[MPI_BOR] = REAL_CONSTANT(BOR);
  lh_constants_map[MPI_LXOR] = REAL_CONSTANT(LXOR);
  lh_constants_map[MPI_BXOR] = REAL_CONSTANT(BXOR);
  lh_constants_map[MPI_MAXLOC] = REAL_CONSTANT(MAXLOC);
  lh_constants_map[MPI_MINLOC] = REAL_CONSTANT(MINLOC);
  lh_constants_map[MPI_REPLACE] = REAL_CONSTANT(REPLACE);
  lh_constants_map[MPI_NO_OP] = REAL_CONSTANT(NO_OP);
  lh_constants_map[MPI_DATATYPE_NULL] = REAL_CONSTANT(DATATYPE_NULL);
  lh_constants_map[MPI_BYTE] = REAL_CONSTANT(BYTE);
  lh_constants_map[MPI_PACKED] = REAL_CONSTANT(PACKED);
  lh_constants_map[MPI_CHAR] = REAL_CONSTANT(CHAR);
  lh_constants_map[MPI_SHORT] = REAL_CONSTANT(SHORT);
  lh_constants_map[MPI_INT] = REAL_CONSTANT(INT);
  lh_constants_map[MPI_LONG] = REAL_CONSTANT(LONG);
  lh_constants_map[MPI_FLOAT] = REAL_CONSTANT(FLOAT);
  lh_constants_map[MPI_DOUBLE] = REAL_CONSTANT(DOUBLE);
  lh_constants_map[MPI_LONG_DOUBLE] = REAL_CONSTANT(LONG_DOUBLE);
  lh_constants_map[MPI_UNSIGNED_CHAR] = REAL_CONSTANT(UNSIGNED_CHAR);
  lh_constants_map[MPI_SIGNED_CHAR] = REAL_CONSTANT(SIGNED_CHAR);
  lh_constants_map[MPI_UNSIGNED_SHORT] = REAL_CONSTANT(UNSIGNED_SHORT);
  lh_constants_map[MPI_UNSIGNED_LONG] = REAL_CONSTANT(UNSIGNED_LONG);
  lh_constants_map[MPI_UNSIGNED] = REAL_CONSTANT(UNSIGNED);
  lh_constants_map[MPI_FLOAT_INT] = REAL_CONSTANT(FLOAT_INT);
  lh_constants_map[MPI_DOUBLE_INT] = REAL_CONSTANT(DOUBLE_INT);
  lh_constants_map[MPI_LONG_DOUBLE_INT] = REAL_CONSTANT(LONG_DOUBLE_INT);
  lh_constants_map[MPI_LONG_INT] = REAL_CONSTANT(LONG_INT);
  lh_constants_map[MPI_SHORT_INT] = REAL_CONSTANT(SHORT_INT);
  lh_constants_map[MPI_2INT] = REAL_CONSTANT(2INT);
  lh_constants_map[MPI_WCHAR] = REAL_CONSTANT(WCHAR);
  lh_constants_map[MPI_LONG_LONG_INT] = REAL_CONSTANT(LONG_LONG_INT);
  lh_constants_map[MPI_LONG_LONG] = REAL_CONSTANT(LONG_LONG);
  lh_constants_map[MPI_UNSIGNED_LONG_LONG] = REAL_CONSTANT(UNSIGNED_LONG_LONG);
  lh_constants_map[MPI_2COMPLEX] = REAL_CONSTANT(2COMPLEX);
  lh_constants_map[MPI_2DOUBLE_COMPLEX] = REAL_CONSTANT(2DOUBLE_COMPLEX);
  lh_constants_map[MPI_CHARACTER] = REAL_CONSTANT(CHARACTER);
  lh_constants_map[MPI_LOGICAL] = REAL_CONSTANT(LOGICAL);
  lh_constants_map[MPI_LOGICAL1] = REAL_CONSTANT(LOGICAL1);
  lh_constants_map[MPI_LOGICAL2] = REAL_CONSTANT(LOGICAL2);
  lh_constants_map[MPI_LOGICAL4] = REAL_CONSTANT(LOGICAL4);
  lh_constants_map[MPI_LOGICAL8] = REAL_CONSTANT(LOGICAL8);
  lh_constants_map[MPI_INTEGER] = REAL_CONSTANT(INTEGER);
  lh_constants_map[MPI_INTEGER1] = REAL_CONSTANT(INTEGER1);
  lh_constants_map[MPI_INTEGER2] = REAL_CONSTANT(INTEGER2);
  lh_constants_map[MPI_INTEGER4] = REAL_CONSTANT(INTEGER4);
  lh_constants_map[MPI_INTEGER8] = REAL_CONSTANT(INTEGER8);
  lh_constants_map[MPI_REAL] = REAL_CONSTANT(REAL);
  lh_constants_map[MPI_REAL4] = REAL_CONSTANT(REAL4);
  lh_constants_map[MPI_REAL8] = REAL_CONSTANT(REAL8);
  lh_constants_map[MPI_REAL16] = REAL_CONSTANT(REAL16);
  lh_constants_map[MPI_DOUBLE_PRECISION] = REAL_CONSTANT(DOUBLE_PRECISION);
  lh_constants_map[MPI_COMPLEX] = REAL_CONSTANT(COMPLEX);
  lh_constants_map[MPI_COMPLEX8] = REAL_CONSTANT(COMPLEX8);
  lh_constants_map[MPI_COMPLEX16] = REAL_CONSTANT(COMPLEX16);
  lh_constants_map[MPI_COMPLEX32] = REAL_CONSTANT(COMPLEX32);
  lh_constants_map[MPI_DOUBLE_COMPLEX] = REAL_CONSTANT(DOUBLE_COMPLEX);
  lh_constants_map[MPI_2REAL] = REAL_CONSTANT(2REAL);
  lh_constants_map[MPI_2DOUBLE_PRECISION] = REAL_CONSTANT(2DOUBLE_PRECISION);
  lh_constants_map[MPI_2INTEGER] = REAL_CONSTANT(2INTEGER);
  lh_constants_map[MPI_INT8_T] = REAL_CONSTANT(INT8_T);
  lh_constants_map[MPI_UINT8_T] = REAL_CONSTANT(UINT8_T);
  lh_constants_map[MPI_INT16_T] = REAL_CONSTANT(INT16_T);
  lh_constants_map[MPI_UINT16_T] = REAL_CONSTANT(UINT16_T);
  lh_constants_map[MPI_INT32_T] = REAL_CONSTANT(INT32_T);
  lh_constants_map[MPI_UINT32_T] = REAL_CONSTANT(UINT32_T);
  lh_constants_map[MPI_INT64_T] = REAL_CONSTANT(INT64_T);
  lh_constants_map[MPI_UINT64_T] = REAL_CONSTANT(UINT64_T);
  lh_constants_map[MPI_AINT] = REAL_CONSTANT(AINT);
  lh_constants_map[MPI_OFFSET] = REAL_CONSTANT(OFFSET);
  lh_constants_map[MPI_C_BOOL] = REAL_CONSTANT(C_BOOL);
  lh_constants_map[MPI_C_COMPLEX] = REAL_CONSTANT(C_COMPLEX);
  lh_constants_map[MPI_C_FLOAT_COMPLEX] = REAL_CONSTANT(C_FLOAT_COMPLEX);
  lh_constants_map[MPI_C_DOUBLE_COMPLEX] = REAL_CONSTANT(C_DOUBLE_COMPLEX);
  lh_constants_map[MPI_C_LONG_DOUBLE_COMPLEX] = REAL_CONSTANT(C_LONG_DOUBLE_COMPLEX);
  lh_constants_map[MPI_CXX_BOOL] = REAL_CONSTANT(CXX_BOOL);
  lh_constants_map[MPI_CXX_COMPLEX] = REAL_CONSTANT(CXX_COMPLEX);
  lh_constants_map[MPI_CXX_FLOAT_COMPLEX] = REAL_CONSTANT(CXX_FLOAT_COMPLEX);
  lh_constants_map[MPI_CXX_DOUBLE_COMPLEX] = REAL_CONSTANT(CXX_DOUBLE_COMPLEX);
  lh_constants_map[MPI_CXX_LONG_DOUBLE_COMPLEX] = REAL_CONSTANT(CXX_LONG_DOUBLE_COMPLEX);
  lh_constants_map[MPI_COUNT] = REAL_CONSTANT(COUNT);
  lh_constants_map[MPI_ERRORS_ARE_FATAL] = REAL_CONSTANT(ERRORS_ARE_FATAL);
  lh_constants_map[MPI_ERRORS_RETURN] = REAL_CONSTANT(ERRORS_RETURN);
}
