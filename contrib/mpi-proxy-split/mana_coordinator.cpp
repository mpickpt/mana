...
static bool noRankInState(const ClientToStateMap& clientStates, long int size,
                          phase_t state);
static bool allInPhase1orCsNoTriv(const ClientToStateMap& clientStates,
                                  long int size);
...
#if 0
  if (allInPhase1orCsNoTriv(clientStates, status.numPeers)) {
    freeAllPhase1(clientStates, status.numPeers);
    goto done;
  }
#endif
#endif
...
static bool
allInPhase1orCsNoTriv(const ClientToStateMap& clientStates, long int size)
{
  for (RankKVPair c : clientStates) {
...
static void
freeAllPhase1(const ClientToStateMap& clientStates, long int size)
{
...
#ifdef HYBRID_2PC
  // If some rank anywhere is in FINISHED_PHASE2, gives it a
  // FREE_PASS
  for (RankKVPair c : clientStates) {
    if (queries[c.second.rank] != NONE) {
      continue; // the message is already decided
